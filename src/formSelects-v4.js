...
(function(layui, window, factory) {
		...
		$((target ? target : `select[${NAME}]`)).each((index, select) => {
			let othis = $(select),
				...
				options = {
					...
					searchUrl: this.isSearch ? othis.attr(SEARCH) : null,
					...
					placeholder: this.optionsFirst ? (
						this.optionsFirst.value ? TIPS : (this.optionsFirst.innerHTML || TIPS)
					) : TIPS,
					btns: this.radio ? [quickBtns[1]] : [quickBtns[0], quickBtns[1], quickBtns[2]],
				},
	...
	let Select4 = function() {
		...
		if(Vue) {
			let VueFormSelects = {};
			VueFormSelects.install = (Vue, options) => {
				Vue.directive('xm-select', (el, binding, vnode, oldVnode) => {
					...
					setTimeout(() => {
						...
						if(vnode.children._isVList && common.check(id)){
							let config = ajaxs[id] ? ajaxs[id] : ajax;
							this.data(id, 'local', {
								arr: vnode.children.map((item) => {
									let dt = {};
									if(item.data && item.data.domProps){
										dt[config['keyName']] = item.data.domProps.textContent;
										dt[config['keyVal']] = item.data.domProps.value;
									}
									item.data && item.data.attrs && (dt[config['keyDis']] = item.data.attrs.disabled);
									return dt;
								})
							}).value(id, vals);
						}
