...
1. 不在删除原始`select`, 保留在dom结构上
2. 修改`render`, 精简代码
3. 修改`on`, 新增参数, 可变为滞后监听, 能获取当前已选择数据
4. 兼容vue, 详情查看vue模块
5. 新增下拉任意, 详情查看plugin模块 -- 只是起了一个名字, 还没有做
6. config中新增response参数, 可自定义返回参数

!> bug修复

1. 修改maxTips在未渲染的情况下设置, 渲染后不生效的问题
2. 修改filter在未渲染的情况下设置, 渲染后不生效的问题
