...
(function (layui, window, factory) {
  ...
  Common.prototype.search = function (id, e, searchUrl, call) {
    ...
    if (searchUrl) {
      ...
      reElem.find("dl .".concat(DD_HIDE)).removeClass(DD_HIDE); //遍历选项, 选择可以显示的值
  ...
  Common.prototype.one = function (target) {
    ...
    $(target ? target : document).off('click', "dl.".concat(DL)).on('click', "dl.".concat(DL), function (e) {
      ...
      if (othis.is(".".concat(LINKAGE)) || othis.parents(".".concat(LINKAGE))[0]) {
        ...
        nextGroup.find("li[pid=\"".concat(othis.attr('xm-value'), "\"]")).removeClass('xm-select-linkage-hide'); // jh-move-to-start
        ...
        _this11.handlerLabel(_id, null, isAdd, val); // jh-move-to-end
        //如果没有下一个group, 或没有对应的值


        if (!nextGroup[0] || nextGroup.find("li:not(.xm-select-linkage-hide)").length == 0) {// jh-move-from-start
          // jh-move-from-end
        } else {
  ...
  Common.prototype.handlerLabel = function (id, dd, isAdd, oval, notOn) {
    ...
    var dl = $("dl[xid=\"".concat(id, "\"]"));
    isAdd ? (dd && dd[0] ? (dd.addClass(THIS), dd.removeClass(TEMP)) : dl.find('.xm-select-linkage')[0] && this.linkageAdd(id, val), // jh-add-start
    function (self) {
    ...
    this.addLabel(id, div, val), vals.push(val)) : (dd && dd[0] ? dd.removeClass(THIS) : dl.find('.xm-select-linkage')[0] && this.linkageDel(id, val),
    /* (function(self) {
      if (val.name.indexOf("/") < 0) {
        return
      }
        let nameList = val.name.split("/")
      nameList.pop()
      let newName = nameList.join("/")
      for (let i = 0; i < vals.length; i++) {
        let itemName = vals[i].name
        if (itemName !== val.name && itemName.indexOf(newName) === 0) {
          console.log(vals[i].name, newName)
          return
        }
      }
      ...
      self.addLabel(id, div, newVal)
      vals.push(newVal)
    }(this)), */
    this.delLabel(id, div, val), this.remove(vals, val));
