...
(function (layui, window, factory) {
  ...
  var $ = layui.jquery || $,
    ...
    value: function value(name) {
      ...
      var vals = arguments.length > 2 ? arguments[2] : undefined;
      ...
      if (name && vals && Array.isArray(vals)) {
        ...
        if (options) {
          ...
          vals.forEach(function (val) {
            dl.find("dd:not(.layui-disabled)[lay-value='".concat(val, "']")).click();
          });
          options.on = on;
        }
    ...
    data: {
      ...
      DEFAULT_OPTIONS: {
        ...
        //xm-select="xxx"
        type: 1,
        //显示模式, 1:layui-this, 2:checkbox, 3:icon
        icon: {
          class: 'layui-icon-ok',
          text: '&#xe618;'
        },
    ...
    methods: {
      init: function init(options, clone) {
        ...
        var _ref = ["xm-".concat(options.name), commons.methods.getDom(options, true)],
            filter = _ref[0],
            dom = _ref[1];
        ...
        if (options.data && options.data.arr) {
          ...
          for (var i in os.arr) {
            ...
            if (db.arr && Array.isArray(db.arr)) {
              html += "<optgroup label=\"".concat(db.name, "\">");

              for (var j in db.arr) {
                var gdb = db.arr[j];
                html += "<option value=\"".concat(gdb[os.val], "\" ").concat(gdb[os.selected] ? 'selected="selected"' : '', " ").concat(gdb[os.disabled] ? 'disabled="disabled"' : '', ">").concat(gdb[os.name], "</option>");
              }

              html += "</optgroup>";
            } else {
              html += "<option value=\"".concat(db[os.val], "\" ").concat(db[os.selected] ? 'selected="selected"' : '', " ").concat(db[os.disabled] ? 'disabled="disabled"' : '', ">").concat(db[os.name], "</option>");
            }
          }
        ...
        if (dom.parent().hasClass(commons.data.pclass)) {
          ...
          dom.wrap("<div class=\"layui-form ".concat(commons.data.pclass, "\" lay-filter=\"").concat(filter, "\"></div>"));
        }
        ...
        commons.methods.formRender('select', filter, true); //1.去掉layui的原始渲染

        commons.methods.getDom(options).next().addClass(commons.data.vclass); //2.
        ...
        commons.data.values.delete(options.name); //3.渲染input

        commons.methods.overrideInput(options);
        commons.methods.typeInit(options, filter); //4.初始化init
      ...
      addLabel: function addLabel(options, vals) {
        ...
        vals.forEach(function (val) {
          var tips = "fsw=\"".concat(options.name, "\"");
          var _ref2 = [$("<span ".concat(tips, " value='").concat(val.val, "'><font ").concat(tips, ">").concat(val.name, "</font></span>")), $("<i ".concat(tips, " class=\"layui-icon\">&#x1006;</i>"))],
              $label = _ref2[0],
              $close = _ref2[1];
          $label.append($close);
          ipt.append($label);
      ...
      delLabel: function delLabel(options, vals) {
        var ipt = commons.methods.getIpt(options);
        vals.forEach(function (val) {
          ipt.find("span[value='".concat(val.val, "']:first")).remove();
        });
      },
      showPlaceholder: function showPlaceholder(options) {
        ...
        if (!vals.length) {
          var _ref3 = ["fsw=\"".concat(options.name, "\""), commons.methods.getIpt(options)],
              tips = _ref3[0],
              ipt = _ref3[1];
          ...
          if (!ipt.find('.xm-select-empty').length) {
            ...
            var cls = "fsw=\"".concat(options.name, "\"");
            var span = $("<span ".concat(cls, " class=\"xm-select-empty\">").concat(_tips, "</span>"));
            ipt.append(span);
          }
        }
      },
      valHandler: function valHandler(options, val, isAdd, isShow) {
        var vals = commons.methods.getValues(options);
        var dd = commons.methods.getDiv(options).find("dl dd[lay-value='".concat(val.val, "']"));
      ...
      typeInit: function typeInit(options, filter) {
        ...
        if (options.type == 2) {
          ...
          div.find('dl dd:not(.layui-select-tips)').each(function (index, target) {
            ...
            $target.text('');
            $target.append("\n\t\t\t\t\t\t\t\t<span lay-filter=\"".concat(filter, "\">\n\t\t\t\t\t\t\t\t\t<input type=\"checkbox\" title=\"").concat(text, "\" lay-skin=\"primary\" ").concat(dis, "> \t\n\t\t\t\t\t\t\t\t</span>\n\t\t\t\t\t\t\t"));
          });
      ...
      typeHandler: function typeHandler(options, dd, isAdd) {
        if (options.type == 3) {
          //对勾
          if (isAdd) {
            var span = $("\n\t\t\t\t\t\t\t\t<span><i class=\"layui-icon ".concat(options.icon.class, "\">").concat(options.icon.text, "</i></span>\t\n\t\t\t\t\t\t\t"));
            dd.append(span);
          } else {
            dd.find('span').remove();
          }
        ...
        isAdd ? dd.addClass("".concat(commons.data.name, "-this")) : dd.removeClass("".concat(commons.data.name, "-this"));
        dd.removeClass('layui-this');
        commons.methods.showPlaceholder(options);
      },
      on: function on(options, filter) {
        form.on("select(".concat(filter, ")"), function (data) {
          if (options.radio) {
            ...
            if (data.value) {
              val = {
                name: commons.methods.getDom(options).find("option[value='".concat(data.value, "']")).text(),
                val: data.value
              };
            ...
            if (data.value) {
              ...
              if (options.delete) {
                ...
                if (_val) {
                  ...
                  ipt.parent().next().find("dd[lay-value='".concat(data.value, "'] .layui-form-checkbox > i")).html('&#xe605;');
      ...
      removeAll: function removeAll(options) {
        ...
        vals.each(function (index, item) {
          options.delete = true;
          div.find("dl dd.xm-select-this:not(.layui-disabled)[lay-value='".concat(item.getAttribute('value'), "']")).click();
        });
        return vals.length;
      },
      removeDefaultClass: function removeDefaultClass(options) {
        ...
        var text = options.radio ? '请选择' : '清空已选择' + (options.max ? ". \u5F53\u524D\u914D\u7F6E: \u6700\u591A\u9009\u62E9 ".concat(options.max, " \u4E2A") : '');
        var html = "<span>".concat(text, "</span>");
        var that = div.find('dl dd.layui-select-tips');

        if (dom.attr('xm-select-search') != undefined) {
          var icon = '&#xe640;';
          html = "<input class=\"layui-input xm-select-input-search\" placeholder=\"\u5173\u952E\u5B57\u641C\u7D22\"><span><i title=\"".concat(text, "\" xm-select-clear=\"").concat(options.name, "\" class=\"layui-icon\">").concat(icon, "</i></span>");
      ...
      handlerDataTable: function handlerDataTable(dom, height) {
        ...
        if (dom.parents('div.layui-table-cell').length) {
          ...
          dom.parents('.layui-table-box').find("tbody tr[data-index='".concat(index, "'] div.layui-table-cell")).css({
            height: height,
            lineHeight: height,
            overflow: 'unset'
          });
      ...
      overrideInput: function overrideInput(options) {
        var _ref5 = [commons.methods.getDiv(options), commons.methods.getDom(options)],
            _div = _ref5[0],
            _dom = _ref5[1];
        var _ref6 = [_div.find('.layui-select-title input:first'), $("<div class=\"layui-input ".concat(commons.data.name, "\"></div>"))],
            $input = _ref6[0],
            $orinput = _ref6[1];
      ...
      getDom: function getDom(options, isNew) {
        ...
        if (isNew) {
          _dom.set(_name, $("select[".concat(commons.data.name, "='").concat(_name, "']")));
        }
      ...
      getDiv: function getDiv(options) {
        ...
        if (!_div.has(_name)) {
          _div.set(_name, $("select[".concat(commons.data.name, "='").concat(_name, "']")).next());
        }

        return _div.get(_name);
      },
      getIpt: function getIpt(options) {
        return commons.methods.getDiv(options).find("div .".concat(commons.data.name));
      },
      getInitVal: function getInitVal(options) {
        ...
        if (options.radio) {
          var one = vals.map(function (val) {
            var opt = _dom.find("option[value='".concat(val, "']"));
          ...
          result = vals.map(function (val) {
            return {
              name: _dom.find("option[value='".concat(val, "']")).text(),
              val: val + ""
            };
          }).filter(function (val) {
            return val.name && val.val;
      ...
      getOptions: function getOptions(sel) {
        return {
          name: sel.attr("".concat(commons.data.name)),
          type: sel.attr("".concat(commons.data.name, "-type")),
          max: sel.attr("".concat(commons.data.name, "-max")),
          icon: sel.attr("".concat(commons.data.name, "-icon")),
          tips: sel.attr("".concat(commons.data.name, "-placeholder")),
          height: sel.attr("".concat(commons.data.name, "-height")),
          radio: sel.attr("".concat(commons.data.name, "-radio")) != undefined,
          repeat: sel.attr("".concat(commons.data.name, "-repeat")) != undefined
        };
      },
      ...
      autoInit: function autoInit(repeat) {
        $("select[".concat(commons.data.name, "]")).each(function (index, target) {
          var sel = $(target);
      ...
      listenCloseHandler: function listenCloseHandler(e, that, block) {
        if (block.length) {
          ...
          $(".xm-select-parent > select:not([xm-select='".concat(name, "']) + div > dl")).removeClass('xm-select-show').addClass('xm-select-hidn').css('display', 'none');
          ...
          if (that.attr('fsw') != undefined) {
            if (that.is('i')) {
              ...
              commons.methods.getDiv(_options).find("dl dd[lay-value='".concat(val.val, "']")).click();
              setTimeout(function () {
      ...
      listenClose: function listenClose() {
        $(document).on('click', function (e) {
          ...
          setTimeout(function () {
            ...
            commons.methods.retop({
              name: block.find('select[xm-select]').attr('xm-select')
            });
        ...
        $(document).on({
          'input propertychange': function inputPropertychange(e) {
            ...
            dl.find("dd:not(.layui-select-tips):not(:contains('".concat(that.val(), "'))")).addClass('layui-hide');
      ...
      rewriteRender: function rewriteRender() {
        ...
        form.render = function (type, filter, repeat) {
          ...
          if (filter) {
            var sel = $("[lay-filter=".concat(filter, "]")).find("select[".concat(commons.data.name, "]"));
      ...
      loadCss: function loadCss(skin, cs) {
        if (skin) {
          ...
          if ($("style[skin]").length) {
            ...
            sn = $("<style ".concat(skin, " type=\"text/css\"></style>"));
          }
          ...
          $("<style ".concat(commons.data.name, " type=\"text/css\">").concat(styles.css(), "</style>")).insertBefore($('head *:first'));
          var html = '';
          ...
          for (var name in styles.colors) {
            ...
            if (_colors) {
              ...
              html += "<style ".concat(name, " type=\"text/css\">").concat(styles.cssColor(name, _colors[0], _colors[1], _colors[2], _colors[3], _colors[4], _colors[5], _colors[6]), "</style>");
            }
          }

          if (html) {
            $(html).insertAfter($("head style[".concat(commons.data.name, "]")));
          }
        }
      },
      loadLastStyle: function loadLastStyle() {
        $('head').append("<style>".concat(styles.renderCss(), "</style>"));
      },
      ...
      run: function run() {
        ...
        commons.methods.resize('.xm-select:not(.xm-select-height)', function (dom) {
          commons.methods.handlerDataTable(dom, "".concat(dom.clientHeight, "px"));
        });
      }
    }
  },
      styles = {
    colors: {
      //name, spanBgColor, spanColor, iBgColor, iColor, borderColor, tBgColor, tColor
    ...
    cssColor: function cssColor(name, spanBgColor, spanColor, iBgColor, iColor, borderColor, tBgColor, tColor) {
      return "\n\t\t\t\t\tselect[xm-select][xm-select-skin='".concat(name, "'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty){\n\t\t\t\t\t\tbackground: ").concat(spanBgColor, ";\n\t\t\t\t\t\tcolor: ").concat(spanColor, ";\n\t\t\t\t\t\tborder: 1px solid ").concat(borderColor, "\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-skin='").concat(name, "'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty) i{\n\t\t\t\t\t\tbackground: ").concat(iBgColor, ";\n\t\t\t\t\t\tcolor: ").concat(iColor, ";\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-skin='").concat(name, "'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty) i:hover{\n\t\t\t\t\t\topacity:.8;\n\t\t\t\t\t\tfilter:alpha(opacity=80);\n\t\t\t\t\t\tcursor: pointer;\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-skin='").concat(name, "'][xm-select-type='1'] + div dl dd.xm-select-this:not(.layui-disabled),select[xm-select][xm-select-skin='").concat(name, "']:not([xm-select-type]) + div dl dd.xm-select-this:not(.layui-disabled){\n\t\t\t\t\t\tbackground-color: ").concat(tBgColor, ";\n\t\t\t\t\t\tcolor: ").concat(tColor, ";\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-skin='").concat(name, "'][xm-select-type='2'] + div dl dd.xm-select-this:not(.layui-disabled) i{\n\t\t\t\t\t\tbackground-color: ").concat(tBgColor, ";\n\t\t\t\t\t\tborder-color: ").concat(tBgColor, ";\n\t\t\t\t\t\tcolor: ").concat(tColor, ";\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-skin='").concat(name, "'][xm-select-type='3'] + div dl dd.xm-select-this:not(.layui-disabled){\n\t\t\t\t\t\tcolor: ").concat(tBgColor, ";\n\t\t\t\t\t}\n\t\t\t\t\tselect[xm-select][xm-select-type='3'] + div dl dd.xm-select-this i{\n\t\t\t\t\t\tposition: absolute;\n\t\t\t\t\t\tright: 10px;\n\t\t\t\t\t}\n\t\t\t\t");
    },
